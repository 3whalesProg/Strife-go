...
func (vc *VoiceController) AcceptRequest(c *gin.Context) {
	var json struct {
		ChatID uint `json:"chat_id" binding:"required"` // ID чата
	}
	token := c.GetHeader("Authorization")
	...
	socket.AcceptJoinRoom(json.ChatID, claims.ID)
	// Возвращаем информацию о пользователе
	c.JSON(http.StatusOK, gin.H{
		"RoomID": json.ChatID,
	})
...
func (vc *VoiceController) RejectRequest(c *gin.Context) {
	var json struct {
		ChatID uint `json:"chat_id" binding:"required"` // ID чата
	}
	token := c.GetHeader("Authorization")
	...
	socket.RejectJoinRoom(json.ChatID, claims.ID)
	// Возвращаем информацию о пользователе
	c.JSON(http.StatusOK, gin.H{
		"RoomID": json.ChatID,
	})
...
func (vc *VoiceController) RegisterRoutes(router *gin.RouterGroup) {
	...
	router.POST("/acceptRequest", vc.AcceptRequest)
	router.POST("/rejectRequest", vc.RejectRequest)
}
